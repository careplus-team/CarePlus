...
const LeafletMap = dynamic(() => import("../common/leaflet-map"), {
  ssr: false,
  loading: () => (
...
export default function EmergencyRequestModal({
        ...
        try {
          ...
          if ("geolocation" in navigator) {
            watchId = navigator.geolocation.watchPosition(
              ...
              (err) =>
                setErrorMsg(`Location error (${err.code}): ${err.message}`),
              { enableHighAccuracy: true, maximumAge: 0, timeout: 20000 },
            );
          }
        }
  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "ambulance-location" }, (payload) => {
        console.log("Live Location Broadcast:", payload);
        if (payload.payload && payload.payload.lat && payload.payload.lng) {
          setAmbulanceLoc({
            lat: Number(payload.payload.lat),
            lng: Number(payload.payload.lng),
          });
              ...
              <div className="w-full h-64 mt-4 relative border border-slate-200 rounded-lg overflow-hidden">
                ...
                {status === "dispatched" && (
                  <>
                    ...
                    <div
                      className={`absolute bottom-2 left-2 right-2 text-white text-xs py-1 px-2 rounded text-center z-[400] ${isAmbulanceOnline ? "bg-red-600 animate-pulse" : "bg-slate-500"}`}
                    >
                      {isAmbulanceOnline
                        ? "AMBULANCE IS COMING TO YOU"
                        : "AMBULANCE SIGNAL LOST - WAITING FOR UPDATE..."}
                    </div>
                  </>
                )}
              </div>
              ...
              location.lng && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    window.open(
                      `https://www.google.com/maps/dir/?api=1&origin=${ambulanceLoc.lat},${ambulanceLoc.lng}&destination=${location.lat},${location.lng}&travelmode=driving`,
                      "_blank",
                    );
                  }}
                >
