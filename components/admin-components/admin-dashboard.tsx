    ...
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-8 space-y-8">
            ...
            <div className="text-center md:text-left">
              <h2 className="text-3xl md:text-4xl font-bold mb-2">
                ...
                { !dbUserInfo ? (
                  ...
                  <span className="text-blue-100">{dbUserInfo?.name || "Admin"}</span>
                )}
                ! 👋
              </h2>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="lg:hidden">
            ...
            >
                <span>{showOpd ? "Hide OPD Management" : "Show OPD Management"}</span>
                {showOpd ? <ChevronDown size={20} className="rotate-180 transition-transform"/> : <ChevronDown size={20} className="transition-transform"/>}
            </button>
        </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start h-full">
        {/* LEFT SIDE: Main OPD Section */}
        <section className={`lg:col-span-6 w-full ${showOpd ? 'block' : 'hidden lg:block'}`}>
