...
import { Ambulance, List, PlusCircle, UserPlus } from "lucide-react";
...
const EmergencyManagerDashboard = () => {
  ...
  const [initialRequestFetch, setInitialRequestFetch] =
  ...
  // Map Route State
  const [ambulanceLoc, setAmbulanceLoc] = useState<{
    lat: number;
    lng: number;
  } | null>(null);
        ...
        async (payload) => {
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (userDb) {
              ...
              try {
                const docRes = await axios.post("/api/doctor-details-get-api", {
                  email: newItem.userEmail,
                });
                if (docRes.data?.success) userData = docRes.data.data;
              } catch (e) {
                console.error("Error fetching doc details for realtime", e);
              }
            }
            ...
            setDetailsConfig((prev) => {
              if (prev.isOpen && prev.request?.id === payload.new.id) {
                return {
                  ...prev,
                  request: { ...prev.request, ...payload.new },
                };
              }
              return prev;
            });
  ...
  const assignAmbulance = async (requestId: string, ambulanceId: string) => {
    try {
      ...
      if (ambError) {
        ...
        toast.error(
          "Warning: Ambulance status update failed, but request assigned."
        );
      }
      ...
      setRequests((prev) =>
        prev.map((r) =>
          r.id === requestId
            ? {
                ...r,
                assigned: ambulanceId,
                status: "assigned",
                ambulanceId: ambulanceId,
              } // Ensure distinct ID kept
            : r
        )
      );
  ...
  const assignedCount = requests.filter(
    (r) => r.status === "assigned" || r.status === "dispatched"
  ).length;
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
              ...
              {isPendingEmgData ? (
                <>
                  {[1, 2, 3].map((i) => (
                    <div
                      key={i}
                      className="bg-white rounded-lg p-4 border border-slate-100 shadow-sm flex items-center gap-4"
                    >
                      ...
                      >
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 rounded-full bg-amber-50 flex items-center justify-center overflow-hidden border border-amber-100">
                            <Image
                              src={
                                req.user?.profilePicture || "/temp_user.webp"
                              }
              ...
              {isPendingEmgData ? (
                <>
                  {[1, 2].map((i) => (
                    <div
                      key={i}
                      className="bg-white rounded-lg p-4 border border-l-4 border-slate-200 border-l-slate-300 shadow-sm flex items-center gap-4"
                    >
                      <Skeleton className="w-12 h-12 rounded-full" />
                      <div className="flex-1 space-y-2">
                        <Skeleton className="h-4 w-1/3" />
              ...
              {isPendingEmgData ? (
                <>
                  {[1].map((i) => (
                    <div
                      key={i}
                      className="bg-white rounded-lg p-4 border border-slate-100 shadow-sm flex items-center gap-4 opacity-70"
                    >
                      <Skeleton className="w-12 h-12 rounded-full" />
                      <div className="flex-1 space-y-2">
                        <Skeleton className="h-4 w-1/3" />
        ...
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <select
              ...
              onChange={(e) =>
                setAcceptAssignDialog((prev) => ({
                  ...prev,
                  selectedAmbulanceId: e.target.value,
                }))
              }
            >
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() =>
                setAcceptAssignDialog((prev) => ({ ...prev, isOpen: false }))
              }
            >
              Cancel
            </Button>
            <Button
              onClick={handleAcceptAssignConfirm}
      ...
      <AlertDialog
        ...
        onOpenChange={(isOpen) =>
          setDeclineDialog((prev) => ({ ...prev, isOpen }))
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to decline this request? The status will be
              marked as 'Cancelled' and the user will be notified. This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const ManagerRequestDetails = ({
  ...
  const [patientLoc, setPatientLoc] = React.useState<{
    lat: number;
    lng: number;
  }>({
  ...
  useEffect(() => {
    ...
    if (request.ambulance_lat && request.ambulance_lng) {
      setAmbulanceLoc({
        lat: request.ambulance_lat,
        lng: request.ambulance_lng,
      });
    }
    ...
    const channel = client
      ...
      .on("broadcast", { event: "ambulance-location" }, (payload: any) => {
      ...
      .on("broadcast", { event: "patient-location" }, (payload: any) => {
        if (payload.payload && payload.payload.lat && payload.payload.lng) {
      ...
      <div className="h-64 md:h-full min-h-[300px] w-full bg-slate-100 rounded-lg overflow-hidden border border-slate-200 relative">
        ...
        {request.status === "dispatched" && (
          <div className="absolute top-2 right-2 flex flex-col gap-1 items-end z-[400]">
            <div
              className={`px-2 py-1 rounded text-xs shadow font-bold backdrop-blur bg-white/90 ${
                isAmbulanceOnline ? "text-emerald-600" : "text-slate-400"
              }`}
            >
            ...
            <div
              className={`px-2 py-1 rounded text-xs shadow font-bold backdrop-blur bg-white/90 ${
                isPatientOnline ? "text-blue-600" : "text-slate-400"
              }`}
            >
          ...
          {request.ambulanceId && (
            <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-md">
              <p className="text-emerald-800 text-sm font-semibold">
                Assigned Ambulance: {request.ambulanceId}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="flex gap-2">
            {request.status === "pending" && (
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    setDetailsConfig((prev: any) => ({
                      ...prev,
                      isOpen: false,
                    })); // Close details to open accept
            ...
            {request.status === "dispatched" && (
              <Button
                ...
                onClick={() => {
                  window.open(
                    `https://www.google.com/maps/dir/?api=1&origin=${ambulanceLoc?.lat},${ambulanceLoc?.lng}&destination=${request.latitude},${request.longitude}&travelmode=driving`,
                    "_blank"
                  );
                }}
              >
