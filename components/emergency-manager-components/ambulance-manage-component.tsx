...
const AmbulanceManagementDummy = () => {
  ...
  const [assigningOp, setAssigningOp] = useState(false);
  const [removingOp, setRemovingOp] = useState(false);
  ...
  const fetchOperators = async () => {
    try {
      const res = await fetch("/api/ambulance-operators-api");
      const result = await res.json();
      if (result.success) {
  ...
  const handleAssignOperator = async () => {
    if (!selectedAmbulance || !assignOperatorEmail) return;
    setAssigningOp(true);
    try {
      const res = await fetch("/api/ambulance-operator-assign-api", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          operatorEmail: assignOperatorEmail,
      ...
      if (result.success) {
        const updatedAmbulances = ambulances.map((amb) =>
          amb.id === selectedAmbulance.id
            ? { ...amb, assigned_operator_email: assignOperatorEmail }
            : // clear from previous ambulance if operator was already assigned somewhere
              amb.assigned_operator_email === assignOperatorEmail
              ? { ...amb, assigned_operator_email: null }
              : amb,
        );
        setAmbulances(updatedAmbulances);
        setSelectedAmbulance({
          ...selectedAmbulance,
          assigned_operator_email: assignOperatorEmail,
        });
        setAssignOperatorEmail("");
        await fetchOperators();
      } else {
        alert("Failed to assign operator: " + result.message);
      }
    } catch {
      alert("Error assigning operator");
    } finally {
      setAssigningOp(false);
    }
  };
  ...
  const handleRemoveOperator = async () => {
    ...
    setRemovingOp(true);
    try {
      const res = await fetch("/api/ambulance-operator-unassign-api", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ambulanceId: selectedAmbulance.id }),
      });
      const result = await res.json();
      if (result.success) {
        const updatedAmbulances = ambulances.map((amb) =>
        ...
        alert("Failed to remove operator: " + result.message);
      }
    } catch {
    ...
    <div className="min-h-screen bg-gray-50/50 p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                    ...
                    <div className="relative w-full sm:w-32 h-48 sm:h-24 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0 shadow-inner group-hover:scale-[1.02] transition-transform duration-300">
                      ...
                      <div className="absolute top-2 right-2 sm:hidden">
                        <span
                          className={`px-2 py-1 rounded text-[10px] font-bold border shadow-sm ${getStatusColor(ambulance.availbility)}`}
                        >
                    ...
                    <div className="flex-1 w-full">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="hidden sm:block text-right">
                          <div
                            className={`px-3 py-1.5 text-[10px] md:text-xs rounded-full border flex items-center justify-center gap-1.5 font-bold uppercase tracking-wider ${getStatusColor(ambulance.availbility)}`}
                          >
                              ...
                              >
                                <div>
                                  ...
                                  <div className="flex items-center gap-2 text-xs font-mono text-gray-600 bg-white p-1.5 rounded border border-dashed border-gray-200">
                                    <span>
                                      Lat:{" "}
                                      {ambulance.active_request.latitude.toFixed(
                                        4,
                                      )}
                                    </span>
                                    ...
                                    <span>
                                      Lng:{" "}
                                      {ambulance.active_request.longitude.toFixed(
                                        4,
                                      )}
                                    </span>
                                  </div>
        ...
        <div className="lg:col-span-5 xl:col-span-4">
          <div
            className={`bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-8 transition-all duration-300 ${selectedAmbulance ? "translate-y-0 opacity-100" : "translate-y-4 opacity-100"} sticky top-6`}
          >
            ...
            {!selectedAmbulance ? (
              <div className="flex flex-col items-center justify-center py-20 text-center text-gray-400">
                ...
                <p className="text-sm opacity-60 mt-1">
                  Click on any ambulance card from the list
                </p>
              </div>
            ) : (
                ...
                <div className="relative w-full h-48 rounded-2xl overflow-hidden mb-6 shadow-md group">
                  ...
                  <div className="absolute bottom-4 left-4 text-white">
                    ...
                    <p className="text-2xl font-bold font-mono tracking-tight">
                      {selectedAmbulance.license_plate}
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white rounded-md border text-sm text-gray-600 shadow-sm">
                        ...
                        <span className="font-semibold">
                          {selectedAmbulance.patient_capacity}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 bg-gray-50 rounded-2xl border border-gray-100">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selectedAmbulance.medical_equipment
                        .split(",")
                        .map((item, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 bg-white border rounded-full text-sm font-medium text-gray-600 shadow-sm"
                          >
                            {item.trim()}
                          </span>
                        ))}
                  ...
                  <div className="p-5 bg-gray-50 rounded-2xl border border-gray-100 space-y-3">
                    ...
                    {selectedAmbulance.assigned_operator_email ? (
                      <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm">
                        ...
                        <button
                          ...
                          disabled={removingOp}
                          className="ml-2 shrink-0 flex items-center gap-1 text-xs text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1 rounded-lg transition-colors disabled:opacity-50"
                        >
                          <UserMinus className="w-3.5 h-3.5" />
                          {removingOp ? "Removing…" : "Remove"}
                        </button>
                      </div>
                    ) : (
                    ...
                    <div className="space-y-2 pt-1">
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          onChange={(e) =>
                            setAssignOperatorEmail(e.target.value)
                          }
                          className="flex-1 min-w-0 border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
                        >
                          ...
                          {operators
                            .filter(
                              (op) =>
                                !op.ambulanceId ||
                                op.ambulanceId === selectedAmbulance.id,
                            )
                            .map((op) => (
                        ...
                        <button
                          ...
                          disabled={!assignOperatorEmail || assigningOp}
                          className="shrink-0 flex items-center gap-1. px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <UserPlus className="w-4 h-4" />
                          {assigningOp ? "Assigning…" : "Assign"}
                        </button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      ...
                      className={`h-10 rounded-xl flex items-center justify-center gap-2 border transition-all duration-200 ${
                        selectedAmbulance.availbility === "Maintenance" ||
                        selectedAmbulance.availbility === "On-Mission"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              ambulance from the fleet and remove its data from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
