...
const AmbulanceOperatorSecurity = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    startTransition(async () => {
      ...
      if (!currentUserData?.data?.claims?.email) {
        ...
        // Check if user is ambulance operator
        const adminCheckData = await axios.post(
          "/api/check-ambulance-opeartor",
          {
            email: currentUserData?.data?.claims?.email,
          }
        );
        console.log("ambulance operator check data", adminCheckData.data);
        if (!adminCheckData.data.isAdmin) {
          router.push("/login");
        } else {
          console.log("User is an ambulance operator");
        ...
        <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
          <div className="bg-transparent rounded-lg  p-8 max-w-sm w-full text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">
              Verifying Ambulance Operator Access
            </h2>
...
export default AmbulanceOperatorSecurity;
